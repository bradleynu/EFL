...
class AAWP_API {
    ...
    public function get_products( $asins = array(), $args = array() ) {
        ...
        foreach ( $chunks as $chunk ) {
            ...
            if ( ! empty( $products['error'] ) && count( $chunk ) === 1 ) { // When the single asin is attempted to fetch/renew.
                aawp_handle_api_product_error_response( $chunk[0], $products['error'] );
            }

            /*
             * Error returned by API, try looping items individually
             */
            if ( isset ( $products['error'] ) && sizeof( $chunk ) > 1 ) {
