...
function aawp_check_dependencies() {
    ...
    // Check PHP modules
    if ( !function_exists('curl_version') || ! extension_loaded('mbstring') )
        return false;
...
function aawp_get_amazon_associates_links() {
    ...
    $associate_links = array(
        ...
        'eg' => 'https://affiliate-program.amazon.eg/',
...
function aawp_get_page_url( $slug ) {
    ...
    switch ( $slug ) {
        ...
        case 'flyout-contact':
            ...
            break;
        case 'telemetry-data':
            $url = $lang_de ? 'https://aawp.de/docs/article/telemetrie-daten/' : 'https://getaawp.com/docs/article/telemetry-data/';
            break;
        case 'docs:aawp-api':
            $url = $lang_de ? 'https://aawp.de/docs/article/aawp-api/' : 'https://getaawp.com/docs/article/aawp-api/';
            break;
    }
...
function aawp_get_credits_link() {
    ...
    $link = sprintf( wp_kses( __( 'This product presentation was made with <a href="%s" rel="nofollow" target="_blank" title="Amazon WordPress Plugin">AAWP</a> plugin.', 'aawp' ), array(  'a' => array( 'href' => array(), 'rel' => array(), 'target' => array(), 'title' => array() ) ) ), esc_url( $url ) );
...
function aawp_format_price_currency( $price ) {
    ...
    if ( in_array( $country, $euro_countries ) ) {
        ...
        $prefix = aawp_get_currency_symbol( 'CAD' );
...
function aawp_execute_database_garbage_collection() {
    ...
    if ( $deleted_products > 0 || $deleted_lists > 0 ) {
        aawp_log( 'Database Garbase Collection', sprintf( esc_html__( '%s products and %s lists removed from the database.', 'aawp' ), '<code>' . $deleted_products . '</code>', '<code>' . $deleted_lists . '</code>' ) );
    }
}
...
/**
 * Fallback mechanism for AAWP API. Checks if AAWP API field is checked & tracking ID is not empty.
 *
 * @since 3.30.0
 *
 * @return bool
 */
function is_aawp_api_eligible() {

    $api_settings = aawp_get_options('api');

    return ! empty( $api_settings['aawp_api_connect'] ) && ! empty( $api_settings['associate_tag'] ) && empty( $api_settings['status'] );
}

/**
 * Is ready to set the cookie?
 *
 * @since 3.30.8
 *
 * @return bool
 */
function is_aawp_cookie_eligible() {

	return ! aawp_is_user_admin() && (
		( ! empty( get_option( 'aawp_clicks_settings' )['enable'] ) && ! empty( get_option( 'aawp_clicks_settings' )['country'] ) ) ||
		( ! empty( aawp_get_option( 'geotargeting', 'functions' ) ) && ! empty( aawp_get_option( 'multiple_stores', 'api' ) ) && 'shortened' !== aawp_get_option( 'affiliate_links', 'general' ) )
	);
}

/**
 * Update the .htaccess file within the plugin directory based on the "proxy enabled" setting.
 *
 * This function checks the "proxy enabled" setting and dynamically modifies the .htaccess file
 * within the plugin directory to allow or deny access to the "image.php" file based on the setting's value.
 *
 * @param bool $is_enabled Is Image Proxy enabled or not.
 *
 * @since 3.30.11
 */
function aawp_create_image_proxy_htaccess( $is_enabled ) {

    $htaccess_file = AAWP_PLUGIN_DIR . '/public/.htaccess';

    // If the proxy setting is disabled, add blocking rules to .htaccess.
    $htaccess_content = "<FilesMatch \"image.php\">\n";
    $htaccess_content .= "    Order Allow,Deny\n";
    $htaccess_content .= $is_enabled ? "    Allow from all\n" : "    Deny from all\n";
