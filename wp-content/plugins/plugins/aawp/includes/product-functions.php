 ...
 * @param $products
 * @param array $args
 * @param bool $force Renew product even if it's status is not active.
 *
 * @return int
 */
function aawp_renew_products( $products, $args = array(), $force = true ) {
    ...
    foreach ( $products as $product_index => $product ) {
        ...
        if ( false === $force && 'active' !== $product['status'] ) {
            continue;
        }
    ...
    /**
     * Renew with our API if Amazon API couldn't.
     *
     * @since 3.30.0
     */
    if ( is_aawp_api_eligible() ) {

        if ( empty( get_transient( 'aawp_products_api_error' ) ) ) {
            $api = new \AAWP\API\Product;
            $renewed = $api->renew( $product_asins );
...
function aawp_update_product_status_by_asin( $asin, $new_status ) {
    ...
    if ( ! empty( $product['id'] ) ) {
        ...
        aawp_update_product( $product['id'], $data );
    }
}