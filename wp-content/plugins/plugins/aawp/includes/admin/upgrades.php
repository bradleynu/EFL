...
function aawp_admin_plugin_upgrades() {
    ...
    if ( ! empty( $aawp_version_installed ) ) {
        ...
        // 3.21.0
        if ( version_compare( $aawp_version_installed, '3.21.0', '<' ) ) {
            aawp_admin_plugin_update_pre_3_21_0_action();
        }

        // 3.30.3
        if ( version_compare( $aawp_version_installed, '3.30.3', '<' ) ) {
            aawp_admin_plugin_update_pre_3_30_3_action();
        }

        // 3.30.4
        if ( version_compare( $aawp_version_installed, '3.30.4', '<' ) ) {
            aawp_admin_plugin_update_pre_3_30_4_action();
        }

        // 3.31.0
        if ( version_compare( $aawp_version_installed, '3.31.0', '<' ) ) {
            aawp_admin_plugin_update_pre_3_31_0_action();
        }

        // 3.32.0
        if ( version_compare( $aawp_version_installed, '3.32.0', '<' ) ) {
            aawp_admin_plugin_update_pre_3_32_0_action();
        }
    }
    /* ---------------------------------------------------------- */
    ...
    ( new \AAWP\API\UsageTracking() )->api_call();
...
/**
 * Pre v3.32.0 upgrade handler. Support tab is deprecated. Migrate the options to "Tools" page.
 *
 * @since 3.32.0
 */
function aawp_admin_plugin_update_pre_3_32_0_action() {

    $tools = get_option( 'aawp_tools', [] );

    if ( ! empty( get_option('aawp_support')['uninstall_remove_data'] ) ) {
        $tools['uninstall_aawp'] = 1;
    }

    if ( ! empty( get_option('aawp_support')['disable_wp_cron'] ) ) {
        $tools['deactivate_as'] = 1;
    }
...
/**
 * Pre v3.31.0 upgrade handler. The following option is no longer needed as we're now storing error in transient.
 *
 * @since 3.31.0
 */
function aawp_admin_plugin_update_pre_3_31_0_action() {
    delete_option( 'aawp_products_api_error' );
}

/**
 * Pre v3.30.4 upgrade handler
 */
function aawp_admin_plugin_update_pre_3_30_4_action() {

    if ( class_exists( 'ActionScheduler_DBStore' ) ) {
        \ActionScheduler_DBStore::instance()->cancel_actions_by_hook( 'aawp_remove_aawp_api_products' );
    }
}

/**
 * Pre v3.30.3 upgrade handler
 */
function aawp_admin_plugin_update_pre_3_30_3_action() {

    $api_settings = aawp_get_options( 'api' );
    $api_settings['aawp_api_connect'] = $api_settings['connect'];

    unset( $api_settings['connect'] );

    aawp_update_options( 'api', $api_settings );
}

/**
 * Pre v3.21.0 upgrade handler
 */
function aawp_admin_plugin_update_pre_3_21_0_action() {
