...
class License extends Functions {
	...
	public function __construct() {
		...
		$this->options['licensing']   = get_option( 'aawp_licensing', [] );
		...
		add_action( 'aawp_admin_support_core_version', [ &$this, 'admin_support_core_version_update_info' ] );
		add_filter( 'aawp_admin_notices', [ &$this, 'admin_notice_update_info' ] );
	}
	...
	public function admin_notice_update_info( $notices ) {
		...
		// Manual update info
		$message  = sprintf( wp_kses( __( '<a href="%s">Update available</a>.', 'aawp' ), [ 'a' => [ 'href' => [] ] ] ), admin_url( 'update-core.php' ) );
		$message .= '&nbsp;';
		$message .= sprintf(
			wp_kses(
				__( 'In case the update does not show up correctly or you prefer updating the plugin by hand, please take a look at our <a href="%s" target="_blank">documentation</a> for a manual plugin update.', 'aawp' ),
				[
					'a' => [
						'href'   => [],
						'target' => [],
			...
			( aawp_is_lang_de() ) ? 'https://aawp.de/docs/article/plugin-aktualisieren/' : 'https://getaawp.com/docs/article/updating-the-plugin/'
		);
	...
	public function add_settings() {
		...
		register_setting(
			'aawp_licensing',
			'aawp_licensing',
			[ &$this, 'settings_licensing_callback' ]
		);

		add_settings_section(
			'aawp_licensing_section',
			__( 'Licensing', 'aawp' ),
			[ &$this, 'settings_licensing_section_callback' ],
			'aawp_licensing'
		);
		...
		add_settings_field(
			...
			__( 'Status', 'aawp' ),
			[ &$this, 'settings_licensing_status_render' ],
			'aawp_licensing',
			'aawp_licensing_section'
		);
		...
		add_settings_field(
			'aawp_licensing_key',
			__( 'License key', 'aawp' ),
			[ &$this, 'settings_licensing_key_render' ],
			'aawp_licensing',
			'aawp_licensing_section',
			[ 'label_for' => 'aawp_licensing_key' ]
		);

		add_settings_field(
			'aawp_licensing_server_overwrite',
			__( 'License server', 'aawp' ),
			[ &$this, 'settings_licensing_server_render' ],
			'aawp_licensing',
			'aawp_licensing_section',
			[ 'label_for' => 'aawp_licensing_server_overwrite' ]
		);

		add_settings_field(
			'aawp_licensing_debug',
			__( 'Technical Information', 'aawp' ),
			[ &$this, 'settings_licensing_debug_render' ],
			'aawp_licensing',
			'aawp_licensing_section'
		);
	}
	...
	public function settings_licensing_callback( $input ) {
		...
		if ( ! empty( $input['key'] ) ) {
			...
			$do_license_activation   = isset( $input['verify_license'] ) && ( empty( $input['info'] ) || $input['key'] !== $this->options['licensing']['key'] ) ? true : false;
				...
				if ( ! empty( $server['url'] ) && ! empty( $server['item_id'] ) && ! empty( $server['item_name'] ) ) {
					...
					if ( $do_license_activation ) {
						...
						if ( isset( $license_info['data'] ) ) {
							...
							aawp_log( 'Licensing', 'License Key Activated. <span class="dashicons dashicons-yes"></span> <br/> <pre><code> ' . var_export( $license_info, true ) . ' </code></pre>' );
						}
					} elseif ( $do_license_deactivation ) {
						$license_info = $AAWP_License_Handler->deactivate( $input['key'] );

						aawp_log( 'Licensing', 'License Key Deactivated. <span class="dashicons dashicons-no"></span> <br/> <pre><code> ' . var_export( $license_info, true ) . '</code></pre>' );
	...
	public function settings_licensing_section_callback() {
		...
		?>
			...
			<p>
				...
				<a href="<?php echo aawp_get_page_url( 'docs:license_upgrades' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( '"How to upgrade your license?"', 'aawp' ); ?></a>,&nbsp;
				<a href="<?php echo aawp_get_page_url( 'docs:license_renewals' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( '"How to renew your license?"', 'aawp' ); ?></a>,&nbsp;
				<a href="<?php echo aawp_get_page_url( 'docs:license_server_issues' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( '"License Server – Problems and Fixes"', 'aawp' ); ?></a>
			</p>
			<?php
	}
	...
	public function settings_licensing_server_render() {
		...
		?>
			...
			<p>
				<select id="aawp_licensing_server_overwrite" name="aawp_licensing[server_overwrite]" 
				<?php
				if ( ! aawp_is_user_admin() ) {
					echo 'readonly';}
				?>
				 style="vertical-align: baseline;" disabled>
	...
	private function get_remote_plugin_version() {
		...
		if ( ! empty( $remote_plugin_data ) && ! empty( $remote_plugin_data->new_version ) && is_string( $remote_plugin_data->new_version ) ) {
			...
			set_transient( 'aawp_remote_plugin_version', $remote_plugin_version, 60 * 60 * 24 * 7 );
			// 7 days
