...
class API extends Functions {
	...
	public function add_settings() {
		...
		add_settings_field(
			'aawp_api_status',
			__( 'Status', 'aawp' ),
			[ &$this, 'settings_api_status_render' ],
			'aawp_api',
			'aawp_api_section'
		);
		...
		add_settings_field(
			'aawp_api_key',
			__( 'API Key', 'aawp' ),
			[ &$this, 'settings_api_key_render' ],
			'aawp_api',
			'aawp_api_section',
			[ 'label_for' => 'aawp_api_key' ]
		);

		add_settings_field(
			'aawp_api_secret',
			__( 'API Secret', 'aawp' ),
			[ &$this, 'settings_api_secret_render' ],
			'aawp_api',
			'aawp_api_section',
			[ 'label_for' => 'aawp_api_secret' ]
		);

		/**
		 * Setting for AAWP API connect checkbox.
		 *
		 * @since 3.30
		 */
		add_settings_field(
			'aawp_api_connect',
			__( 'AAWP API', 'aawp' ),
			[ &$this, 'settings_aawp_api_connect_render' ],
			'aawp_api',
			'aawp_api_section',
			[ 'label_for' => 'aawp_api_connect' ]
		);

		add_settings_field(
			'aawp_api_country',
			__( 'Country', 'aawp' ),
			[ &$this, 'settings_api_country_render' ],
			'aawp_api',
			'aawp_api_section',
			[ 'label_for' => 'aawp_api_country' ]
		);

		add_settings_field(
			'aawp_api_associate_tag',
			__( 'Tracking ID', 'aawp' ),
			[ &$this, 'settings_api_associate_tag_render' ],
			'aawp_api',
			'aawp_api_section',
			[ 'label_for' => 'aawp_api_associate_tag' ]
		);
		...
		add_settings_field(
			...
			[ &$this, 'settings_api_debug_render' ],
			'aawp_api',
			'aawp_api_section'
		);
	...
	public function settings_api_callback( $input ) {
		...
		if ( isset( $input['disconnect'] ) ) {
			...
			aawp_log( 'Amazon API', sprintf( __( 'Amazon API Disconnected. %s', 'aawp' ), '<span class="dashicons dashicons-no"></span>' ) );
		}
		...
		if ( ! $disconnect && $verify_credentials && class_exists( 'AAWP_API' ) ) {
			...
			if ( ! empty( $input['key'] ) && ! empty( $input['secret'] ) && ! empty( $input['associate_tag'] ) && ! empty( $input['country'] ) ) {
				...
				if ( true === $credentials_verified ) {
					...
					aawp_log( 'Amazon API', sprintf( __( 'Amazon API Connected. %s', 'aawp' ), '<span class="dashicons dashicons-yes"></span>' ) );

					unset( $input['aawp_api_connect'] );
	...
	public function settings_api_section_callback() {
		...
		<p>
			...
			<?php
			if ( ! empty( $this->options['api']['country'] ) ) {
				...
				?>
				<a href="<?php echo $amazon_associates_link; ?>" title="<?php _e( 'Amazon Associates', 'aawp' ); ?>" target="_blank" rel="noopener noreferrer"><?php _e( 'Amazon Associates', 'aawp' ); ?></a>
				&nbsp;|&nbsp;
				<a href="<?php echo $amazon_product_advertising_api_link; ?>" title="<?php _e( 'Amazon Product Advertising API', 'aawp' ); ?>" target="_blank" rel="noopener noreferrer"><?php _e( 'Amazon Product Advertising API', 'aawp' ); ?></a>
				&nbsp;|&nbsp;
			<?php } ?>
			<a href="<?php echo aawp_get_page_url( 'api_key_checker' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( 'API Key Validator', 'aawp' ); ?></a>
		</p>
		<p>
			<strong><?php _e( 'Documentation:', 'aawp' ); ?></strong>&nbsp;
			<a href="<?php echo aawp_get_page_url( 'docs:api_keys' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( 'Creating API Keys', 'aawp' ); ?></a>,&nbsp;
			<a href="<?php echo aawp_get_page_url( 'docs:api_issues' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( 'Amazon API “not connected” – Frequent causes', 'aawp' ); ?></a>,&nbsp;
			<a href="<?php echo aawp_get_page_url( 'docs:api_requestthrottled' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( '“RequestThrottled” and how to fix it?', 'aawp' ); ?></a>,&nbsp;
			<a href="<?php echo aawp_get_page_url( 'docs:amazon_apiv5' ); ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( 'Amazon API v5 Migration', 'aawp' ); ?></a>
		</p>
	...
	public function settings_api_status_render() {
		...
		} elseif ( is_aawp_api_eligible() && empty( get_transient( 'aawp_products_api_error' ) ) ) {
			echo '<span style="color: green;"><span class="dashicons dashicons-yes aawp-dashicons-inline"></span> ' . __( 'Connected (AAWP API)', 'aawp' ) . '</span>';
		} else {
		...
		if ( ! empty( $error ) ) {
			...
			echo sprintf(
				wp_kses(
					__( 'Please take a look at our article "<a href="%s" target="_blank">Amazon API “not connected” – Frequent causes</a>" in order to solve this problem.', 'aawp' ),
					[
						'a' => [
							'href'   => [],
							'target' => '_blank',
				...
				esc_url( aawp_get_page_url( 'docs:api_issues' ) )
			);
			echo '</p>';
	...
	public function settings_api_key_render() {
		...
		?>
		<input type="<?php echo ( aawp_is_user_admin() ) ? 'text' : 'password'; ?>" id="aawp_api_key" class="regular-text" name="aawp_api[key]" value="<?php echo $key; ?>" 
								<?php
								if ( ! aawp_is_user_admin() ) {
									echo 'readonly';}
								?>
	...
	public function settings_api_secret_render() {
		...
		?>
		<input type="password" id="aawp_api_secret" class="regular-text" name="aawp_api[secret]" value="<?php echo $secret; ?>" 
																												   <?php
																													if ( ! aawp_is_user_admin() ) {
																														echo 'readonly';}
																													?>
	 ...
	 * @since 3.30
	 */
	public function settings_aawp_api_connect_render() {

		$options_api   = aawp_get_options( 'api' );
		$connect       = isset( $options_api['aawp_api_connect'] );
		$connect_error = get_transient( 'aawp_products_api_error' );
		$usage_count   = get_option( 'aawp_products_api_usage_count' );
		$usage_count   = ! empty( $usage_count ) ? $usage_count : 0;
		$licensing     = aawp_get_option( 'info', 'licensing' );
		$limit         = isset( $licensing['data']->license_limit ) ? $licensing['data']->license_limit : 0;
		$usage_limit   = get_option( 'aawp_products_api_usage_limit', $this->get_products_usage_limit( $limit ) );

		?>
			<p>
				<input type="checkbox" id="aawp_api_connect" name="aawp_api[aawp_api_connect]" value="1" <?php echo( $connect ? 'checked' : '' ); ?> />
				<label for="aawp_api_connect"><?php _e( 'Check to retrieve products from the AAWP API instead of the Amazon API.', 'aawp' ); ?></label>
			</p>
			<p>
				<small>
				<?php
				printf(
					esc_html__( 'Activate only if you can\'t retrieve products from the official Amazon API yet. More information about the AAWP API can be found %s. This feature is available exclusively to owners of an active license and for free in the first 3 months after purchase.', 'aawp' ),
			...
			<?php if ( $connect ) : ?>
				<p><button class="button button-secondary" name="aawp_api[connect_test]" ><?php echo esc_html__( 'Test Connection', 'aawp' ); ?></button></p>
					...
					if ( ! empty( $connect_error ) ) {
						   ...
						   echo '<span class="dashicons dashicons-warning" style="color: red;"></span>&nbsp;';
						   echo '<code style="line-height: 25px;">';
						   echo '<strong>ERROR ' . esc_html( $connect_error ) . '</strong> ';
						   echo '</code>';
					} else {
						echo '<p><span style="color: green;"><span class="dashicons dashicons-yes aawp-dashicons-inline"></span> ' . __( 'Connection Successful!', 'aawp' ) . '</span></p>';
					}
					
					if ( '419: Usage Quota Expired.' !== $connect_error ) { // @todo:: return error codes from api so we don't have to compare with error message.
	
						?>
							<p>
								<strong><?php _e( 'API Usage This Month', 'aawp' ); ?></strong><button name="aawp_api[usage_quota]" value="1" class="aawp-api-usage-quota-refresh-button dashicons dashicons-update"></button><br/>
									<progress style="<?php echo $usage_count >= $usage_limit ? 'accent-color: #ff364f;' : 'accent-color: #7bf0ba;'; ?>" value="<?php echo $usage_count; ?>" max="<?php echo $usage_limit; ?>"></progress><br>
								<?php printf( 'Retrieved %s from a max of %s products this month.', $usage_count, $usage_limit ); ?>
							</p>
						<?php
					}
					?>
	...
	public function settings_api_country_render() {
		...
		ksort( $country_tags );
		?>
		<select id="aawp_api_country" name="aawp_api[country]" data-aawp-api-country-selector="true" 
		<?php
		if ( ! aawp_is_user_admin() ) {
			echo 'readonly';}
		?>
	...
	public function settings_api_associate_tag_render() {
		...
		?>
		<input type="text" id="aawp_api_associate_tag" class="regular-text" name="aawp_api[associate_tag]" value="<?php echo $associate_tag; ?>" 
																															 <?php
																																if ( ! aawp_is_user_admin() ) {
																																	echo 'readonly';}
																																?>
	...
	/**
	 * Get Products Usage Limit
	 *
	 * @since 3.30.1
	 */
	private function get_products_usage_limit( $limit ) {
		switch ( true ) {
			case $limit == 1:
				return 50;
			case $limit == 3:
	...
	/**
	 * Send a test call to AAWP API.
	 *
	 * @since 3.30.2
	 */
	public function test_aawp_api() {

		// Bail if it's not an admin page.
		if ( ! is_admin() || ! aawp_is_user_editor() ) {
			return;
		}
		...
		if ( ! isset( $_POST['aawp_api']['connect_test'] ) ) {
			return;
		}
		...
		$endpoint = defined( 'AAWP_API_URL' ) ? AAWP_API_URL . '/test/product' : 'https://api.getaawp.com/v1/test/product';

		$response = wp_remote_post( $endpoint, $options );

		if ( is_wp_error( $response ) ) {
			aawp_log( 'AAWP Products Test API', '<code>' . $response->get_error_message() . '</code>' ); //phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			return;
		}
		...
		if ( isset( $response['response']['code'] ) && 200 === $response['response']['code'] ) {
			...
			$notifications = get_option( 'aawp_notifications' );

			if ( ! empty( $notifications['events'] ) ) {
				foreach ( $notifications['events'] as $key => $notification ) {
					if ( $notification['id'] === 'aawp_api_usage_quota_expiry' ) {
						unset( $notifications['events'][ $key ] );
					}
				}
			}

			update_option( 'aawp_notifications', $notifications );
		...
		if ( isset( $response['response']['code'] ) && isset( $response['body'] ) && 200 !== $response['response']['code'] ) {
			...
			$code    = isset( $body['code'] ) ? $body['code'] : 'ERR_CONNECTION_REFUSED';
			$message = isset( $body['message'] ) ? $body['message'] : 'Unable to connect';
