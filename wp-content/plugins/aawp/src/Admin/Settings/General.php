...
class General extends Functions {
	...
	public function add_settings() {
		...
		add_settings_section(
			...
			[ &$this, 'settings_general_section_callback' ],
			'aawp_general'
		);
		...
		add_settings_field(
			'aawp_shortcode',
			__( 'Shortcode', 'aawp' ),
			[ &$this, 'settings_shortcode_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_shortcode' ]
		);

		/**
		 * Add settings field "Discarded Products".
		 *
		 * @since 3.20
		 */
		add_settings_field(
			'aawp_discarded_products',
			__( 'Discarded Products', 'aawp' ),
			[ &$this, 'settings_discarded_products_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_discarded_products' ]
		);

		add_settings_field(
			'aawp_cache',
			__( 'Cache', 'aawp' ),
			[ &$this, 'settings_cache_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_cache_duration' ]
		);

		add_settings_field(
			'aawp_affiliate_links',
			__( 'Affiliate links', 'aawp' ),
			[ &$this, 'settings_affiliate_links_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_affiliate_links' ]
		);
		...
		add_settings_field(
			...
			[ &$this, 'settings_theme_support_shortcodes_render' ],
			'aawp_general',
			'aawp_general_section'
		);
		...
		add_settings_field(
			...
			[ &$this, 'settings_last_update_format_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_last_update_format' ]
		);

		add_settings_field(
			'aawp_inline_info',
			__( 'Inline info', 'aawp' ),
			[ &$this, 'settings_inline_info_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_inline_info' ]
		);

		add_settings_field(
			'aawp_disclaimer',
			__( 'Disclaimer', 'aawp' ),
			[ &$this, 'settings_disclaimer_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_disclaimer_position' ]
		);

		add_settings_field(
			'aawp_cleanup_shortcode_output',
			__( 'Cleanup shortcode output', 'aawp' ),
			[ &$this, 'settings_cleanup_shortcode_output_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_cleanup_shortcode_output' ]
		);
		...
		add_settings_field(
			'aawp_usage',
			__( 'Telemetry Data', 'aawp' ),
			[ &$this, 'settings_usage_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_usage' ]
		);

		add_settings_field(
			'aawp_credits',
			__( 'Credits', 'aawp' ),
			[ &$this, 'settings_credits_render' ],
			'aawp_general',
			'aawp_general_section',
			[ 'label_for' => 'aawp_credits_position' ]
		);
	}
	...
	/**
	 * Render the dropdown for "Discarded Products" field.
	 *
	 * @since 3.20.
	 */
	public function settings_discarded_products_render() {

		$options = [
			'0'   => __( 'Show Products anyway', 'aawp' ),
			'1'   => __( 'Show product without price', 'aawp' ),
			'2'   => __( 'Hide products', 'aawp' )
		];

		?>
			<select id="aawp_discarded_products" name="aawp_general[discarded_products]">
			<?php foreach ( $options as $value => $label ) { ?>
					<option value="<?php echo esc_attr( $value ); ?>" <?php selected( aawp_get_option( 'discarded_products', 'general' ), $value ); ?>><?php echo esc_html( $label ); ?></option>
				<?php } ?>
			</select>

			<p><?php esc_html_e( 'Specify how to handle products that are no longer available on Amazon.', 'aawp' ); ?></p>
		<?php
	}
	...
	public function settings_cache_render() {
		...
		?>
			...
			<h4><?php _e( 'Database Garbage Collection', 'aawp' ); ?></h4>
			<p>
			...
			printf(
				wp_kses(
					__( 'More information about the database garbage collection can be found in our <a href="%s" target="_blank">documentation</a>.', 'aawp' ),
					[
						'a' => [
							'href'   => [],
							'target' => '_blank',
				...
				esc_url( aawp_get_page_url( 'docs:database_garbage_collection' ) )
			);
			?>
			</p>
			<?php
	...
	/**
	 * Render the setting field for Usages.
	 *
	 * @since 3.22
	 */
	public function settings_usage_render() {

		?>
		<p>
			<input type="checkbox" id="aawp_usage_optin" name="aawp_general[usage_optin]" value="1" <?php echo ! empty( $this->options['general']['usage_optin'] ) ? 'checked' : ''; ?> />
			<label for="aawp_usage_optin"><?php esc_html_e( 'Check to share anonymous telemetry data about plugin usage.', 'aawp' ); ?></label>
		</p>
		<p>
			<?php esc_html_e( 'Sharing telemetry data will help us better understand how our software is being used and identify areas for improvement. Please note that all data collected is anonymous and will not include any personal information. More details can be found', 'aawp' ); ?>
			<?php echo '<a href="'. esc_url( aawp_get_page_url( 'telemetry-data' ) ) .'">' . esc_html__( 'here.', 'aawp' ) . '</a>'; ?>
		</p>
		<?php
	}
	...
	public function settings_credits_render() {
		...
		?>
			...
			printf(
				wp_kses(
					__( 'As a member of our <a href="%s" target="_blank">affiliate partner program</a>, you will receive a commission for every new AAWP customer you refer. If you wish, you can enter your affiliate ID in the field below. As soon as you have activated the "Credits" function above, your Affiliate ID will automatically be appended to the generated link.', 'aawp' ),
					[
						'a' => [
							'href'   => [],
							'target' => '_blank',
				...
				esc_url( aawp_get_page_url( 'affiliates' ) )
			);
			?>
			</p>
			<p>
