...
class ListTable extends WP_List_Table {
	...
	public function column_date_created( $item ) {
		return ! empty( $item['date_created'] ) ? aawp_datetime( strtotime( $item['date_created'] ) ) : '-';
	}
	...
	public function column_date_updated( $item ) {
		return ! empty( $item['date_updated'] ) ? aawp_datetime( strtotime(  $item['date_updated'] ) ) : '-';
	}
		...
		?>
			<ul class="subsubsub">
				<li><a class="<?php echo empty( $_GET['status'] ) || ( ! empty( $_GET['status'] ) && 'all' === $_GET['status'] ) ? 'current' : ''; ?>" 
					...
					<?php
					echo '(' . $this->products_db->count( [], false ) . ')';
					?>
					</a>
				</li> |
				<li><a class="<?php echo ! empty( $_GET['status'] ) && 'active' === $_GET['status'] ? 'current' : ''; ?>" 
					...
					<?php
					echo '(' . $this->products_db->count( ['status' => 'active'], false ) . ')';
					?>
					</a>
				</li> |
				<li><a class="<?php echo ! empty( $_GET['status'] ) && 'invalid' === $_GET['status'] ? 'current' : ''; ?>" 
					...
					<?php
					echo '(' . $this->products_db->count( ['status' => 'invalid'], false ) . ')';
					?>
					</a>
				</li>
			</ui>
	...
	public function display_page() {
		...
		if ( ! empty( $this->items ) || ! empty( $_REQUEST['s'] ) ) {
	...
	public function prepare_items() {
		...
		$total_items = $this->products_db->count();
	...
	public function process_actions() { //phpcs:ignore Generic.Metrics.CyclomaticComplexity.MaxExceeded
		...
		if ( $single_action ) {
			...
			foreach ( $product_ids as $id ) {
				...
				if ( 'delete' === $_REQUEST['action'] ) {
					...
					static $deleted_count = 1;

					if ( 1 === $deleted_count && $deleted ) {
					...
					static $renewed_count = 1;

					if ( 1 === $renewed_count && $renewed ) {
						/* translators: %s Product IDs. */
		...
		set_transient( 'aawp_products_listtable_actions_notice', $query, 5 );

		wp_redirect( admin_url() . 'admin.php?page=aawp-products' );
		exit();
	}
	...
	private function display_notices() {
		...
		$query = get_transient( 'aawp_products_listtable_actions_notice' );
