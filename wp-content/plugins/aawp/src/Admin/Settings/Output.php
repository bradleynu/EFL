...
class Output extends Functions {
	...
	public function __construct() {
		...
		add_action( 'aawp_settings_process', [ $this, 'image_proxy_htaccess' ] );
	}
	...
	public function add_settings() {
		...
		register_setting(
			'aawp_output',
			'aawp_output',
			[ &$this, 'settings_output_callback' ]
		);

		add_settings_section(
			'aawp_output_section',
			__( 'Output settings', 'aawp' ),
			[ &$this, 'settings_output_section_callback' ],
			'aawp_output'
		);
		...
		add_settings_field(
			'aawp_thumbnail',
			__( 'Thumbnail', 'aawp' ),
			[ &$this, 'settings_thumbnail_render' ],
			'aawp_output',
			'aawp_output_section'
		);
		...
		add_settings_field(
			'aawp_custom_css',
			__( 'Custom CSS', 'aawp' ),
			[ &$this, 'settings_custom_css_render' ],
			'aawp_output',
			'aawp_output_section',
			[ 'label_for' => 'aawp_custom_css' ]
		);
	}
	...
	public function settings_title_render() {
		...
		<p>
			...
			<span id="aawp_title_length_extend"
			<?php
			if ( $this->title_length_unlimited == '1' ) {
	...
	public function settings_thumbnail_render() {
		...
		?>
		<h4 class="first"><?php _e( 'Image Proxy', 'aawp' ); ?> (<a href="<?php echo ( aawp_is_lang_de() ) ? 'https://aawp.de/docs/article/dsgvo/' : 'https://getaawp.com/docs/article/gdpr/'; ?>" target="_blank" rel="nofollow noopener noreferrer"><?php _e( 'GDPR relevant', 'aawp' ); ?></a>)</h4>
		...
		<?php } ?>
		<p>
			<input type="checkbox" id="aawp_image_proxy" name="aawp_output[image_proxy]" value="1" <?php echo( $proxy == 1 ? 'checked' : '' ); ?>
																												<?php
																												if ( $proxy_disabled ) {
		...
		<p>
			<small>
				...
				printf(
					wp_kses(
						__( 'For more information please <a href="%s" target="_blank">take a look at this article</a>.', 'aawp' ),
						[
							'a' => [
								'href'   => [],
								'target' => '_blank',
	...
	public function settings_description_render() {
		...
		<p>
			...
			<span id="aawp_description_length_extend"
			<?php
			if ( $this->description_length_unlimited == '1' ) {
				echo ' style="display: none;"';}
			?>
			>
		...
		<p>
			...
			<span id="aawp_teaser_length_extend"
			<?php
			if ( $this->teaser_length_unlimited == '1' ) {
				echo ' style="display: none;"';}
			?>
			>
	...
	public function settings_rating_render() {
		...
		?>
		<p>
			<small><strong><?php _e( 'Note:', 'aawp' ); ?></strong> 
										<?php
										printf(
											wp_kses(
												__( 'In case some ratings do not show up correctly, please <a href="%s" target="_blank">take a look at this article</a>.', 'aawp' ),
												[
													'a' => [
														'href' => [],
														'target' => '_blank',
	...
	public function settings_pricing_render() {
		...
		<p>
			<?php aawp_admin_display_placeholders_note( [ 'price_reduction' ] ); ?>
		</p>
	...
	/**
	 * If the image proxy value is now changed compared to the one stored in the database.
	 * For example: from "enabled to disabled", recreate the .htaccess file.
	 * 
	 * @param $post_data The $_POST data - data before saving the settings.
	 *
	 * @since 3.30.11
	 */
	public function image_proxy_htaccess( $post_data ) {

		if ( 'aawp_output' !== $post_data['option_page'] ) {
			return;
		}

		$option      = aawp_get_option( 'image_proxy', 'output' );
		$image_proxy = isset( $post_data['aawp_output']['image_proxy'] );

		if ( (bool) $option !== $image_proxy ) {
			aawp_create_image_proxy_htaccess( $image_proxy );
		}
	}
}
