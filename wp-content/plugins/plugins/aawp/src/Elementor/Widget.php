...
class Widget extends Widget_Base {
	...
	protected function content_controls() {
		...
		$this->add_control(
			'asin',
			[
				'label'       => esc_html__( 'Enter ASIN', 'aawp' ),
				...
				'classes'     => 'aawp-el-widget-control aawp-asin-input-control',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'raw'             => sprintf(
					'<div class="aawp-products-search-container">
							' . esc_html__( 'Or,', 'aawp' ) . '
							<button class="button button-secondary aawp-table-add-products-search" href="#aawp-modal-table-product-search" data-aawp-modal="true" data-aawp-table-add-products-search="true">
								<span class="dashicons dashicons-search"></span>
									' . esc_html__( 'Search For Product(s)', 'aawp' ) . '
							</button>
						</div>'
				),
				'condition'       => [
					'display_variant' => [ 'box', 'fields', 'links' ],
				]
			]
		);
		...
		$templates = [
			'default'         => esc_html__( 'Default', 'aawp' ),
		...
		$this->add_control(
			'template',
			[
				'label'       => esc_html__( 'Template', 'aawp' ),
				'type'        => Controls_Manager::SELECT,
				'label_block' => false,
				'options'     => $templates,
				'default'     => 'default',
				'condition'   => [
					'display_variant' => [ 'box', 'bestseller', 'new' ],
				],
				'classes'     => 'aawp-el-widget-control',
			]
		);
	...
	protected function render_edit_mode() {
		...
		$asin     = $this->get_settings_for_display( 'asin' );
		$keywords = $this->get_settings_for_display( 'keywords' );
		$table    = $this->get_settings_for_display( 'table' );

		// Render if not configured.
		if ( empty( $asin ) && empty( $keywords ) && empty( $table ) ) {
			echo '<img src=" '. \esc_url( plugins_url( 'assets/img/block-icon.svg', AAWP_PLUGIN_FILE ) ) .'" title="AAWP Icon" />';
			echo '<br/>';
			echo '<small>'. esc_html__( 'Choose your display variant to place product(s).', 'aawp' ) .'</small>';
			return;
		}
