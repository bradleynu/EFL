    ...
    class AAWP_Template_Functions extends AAWP_Functions {
        ...
        public function get_product_image( $size = '' ) {
	        ...
	        if ( '1' == aawp_get_option( 'image_proxy', 'output' ) && ini_get('allow_url_fopen') ) {
                ...
                $expiration_time   = apply_filters( 'aawp_proxy_image_expiration_time', time() + 48 * 3600 ); // URL valid for 48 hours.
        ...
        public function show_advertised_price() {
            ...
            if ( $this->pricing_advertised_price === 'hidden' ||
                (
                    ! empty( $this->item )
                    && 'invalid' === $this->item->get_status()
                    && ! empty( $this->options['general']['discarded_products'] )
                    && $this->options['general']['discarded_products'] === '1'
                ) ) {
                return false;
            }
