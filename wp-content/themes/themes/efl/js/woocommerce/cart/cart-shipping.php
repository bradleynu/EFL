...
?>
<tr class="shipping">
	<th><?php //echo wp_kses_post( $package_name ); ?><?php echo pll_e( 'Costo de envío:', 'efl'); ?></th>
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		...
		<?php elseif ( 1 === count( $available_methods ) ) :  ?>
			<?php
                ...
                $mylabel = str_replace('Costo: ', '', wc_cart_totals_shipping_method_label( $method ));

				printf( '%3$s <input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d" value="%2$s" class="shipping_method" />', $index, esc_attr( $method->id ), $mylabel );
				do_action( 'woocommerce_after_shipping_rate', $method, $index );
			?>
