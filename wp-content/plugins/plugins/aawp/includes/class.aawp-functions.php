...
if ( !class_exists( 'AAWP_Functions' ) ) {
    ...
    class AAWP_Functions
    {
        ...
        public $title_length_unlimited;
        public $title_length;
        public $title_adding;
        public $image_link_title_adding;
        public $description_length_unlimited;
        public $description_length;
        public $description_html;
        public $teaser;
        public $teaser_length_unlimited;
        public $teaser_length;
        public $pricing_advertised_price;
        public $pricing_advertised_price_hide_unavailability;
        public $pricing_reduction;
        public $check_prime;
        public $star_rating_size;
        public $star_rating_link;
        public $show_reviews;
        public $button_style;
        public $button_icon;
        public $button_text;
        ...
        public function get_items( $keys, $type, $is_list = false, $args = array() ) {
            ...
            // Handle single asins
                ...
                $asins = is_string( $keys ) ? array_map( 'trim', explode( ',', rtrim( $keys, ',' ) ) ) : $keys;
            }
            ...
            if ( is_array( $products_missing ) && sizeof( $products_missing ) > 0 ) {
                ...
                /* 
                 * Fetch product from our own API. NULL means no Official API connection.
                 *
                 * @since 3.30.0
                 *
                 */
                if ( NULL === $products_fetched && is_aawp_api_eligible() ) {
                    $products_fetched = [];
                    $aawp_api = new \AAWP\API\Product;

                    foreach ( $products_missing as $asin ) {
                        $product = $aawp_api->get_product( $asin );

                        if ( ! empty( $product ) && is_array( $product ) ) {
                            $products_fetched[ $asin ] = $product;
                        }
                    }
                }
        ...
        public function get_products_from_cache( $asins = array() ) {
            ...
            $args = array(
                'asin' => $asins,
                'number' => sizeof( $asins ),
                'status' => ! empty( $this->options['general']['discarded_products'] ) && '2' === $this->options['general']['discarded_products'] ? 'active' : ''
            );
        ...
        public function get_products_from_api( $asins = array() ) {
            ...
            foreach ( $products_fetched as $product ) {
                ...
                if ( ! empty( $product->data['ASIN'] ) ) {
                    $valid_asins[] = $product->data['ASIN'];
                }
            ...
            if ( ! empty( $valid_asins ) ) {
                aawp_log( 'Amazon API (Product)', sprintf( wp_kses( _n( 'Fetched product with ASIN <code>%s</code> from the API.', 'Fetched products with ASIN <code>%s</code> from the API.', count( $valid_asins ),  'aawp' ), [ 'code' => [] ] ), implode( ',', $valid_asins ) )  );
            }
        ...
        public function prepare_list_keywords( $keywords ) {
            ...
            $keywords = apply_filters( 'aawp_prepare_list_keywords', $keywords );
