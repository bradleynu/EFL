...
class Frontend {
	...
	public function track_click_v1( $data ) {
		...
		$product_id  = isset( $post_data['productId'] ) ? absint( $post_data['productId'] ) : 0;
		$source_type = isset( $post_data['sourceType'] ) ? sanitize_text_field( wp_unslash( $post_data['sourceType'] ) ) : '';
		$source_id   = isset( $post_data['sourceId'] ) ? absint( $post_data['sourceId'] ) : 0;
		$referer_url = isset( $post_data['refererUrl'] ) ? esc_url( $post_data['refererUrl'] ) : '-';
		$is_widget   = isset( $post_data['isWidget'] ) ? absint( $post_data['isWidget'] ) : 0;
		$tracking_id = isset( $post_data['trackingId'] ) ? sanitize_text_field( wp_unslash( $post_data['trackingId'] ) ) : aawp_get_default_tracking_id();
		$link_type   = isset( $post_data['linkType'] ) ? sanitize_text_field( wp_unslash( $post_data['linkType'] ) ) : '-';
		$country     = isset( $post_data['country'] ) ? sanitize_text_field( wp_unslash( $post_data['country'] ) ) : '';
